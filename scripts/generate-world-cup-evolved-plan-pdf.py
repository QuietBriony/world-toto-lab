...
SNAPSHOT_1637_VOTE_LABEL = "2026-06-22 20:56 JST"
SNAPSHOT_1637_SALES_LABEL = "2026-06-22 20:56 JST"
TOTAL_SALES_1637_YEN = 43_181_300
VOTE_UNITS_1637 = 431_813
RETURN_RATE = 0.5
TIER_DEFS = (
    ("1等", 0, 0.70, True),
    ("2等", 1, 0.15, False),
...
MATCHES_1637 = [
    PlanMatch(1, "エクアドル", "ドイツ", "06/26 05:00", (0.0474, 0.1170, 0.8356), ("2", "0"), "強豪人気 + 第3戦ドロー", "semi"),
    PlanMatch(2, "日本", "スウェーデン", "06/26 08:00", (0.6454, 0.2463, 0.1083), ("1", "0"), "日本勝ち軸 + ドロー", "flex"),
    PlanMatch(3, "ウルグアイ", "スペイン", "06/27 09:00", (0.0730, 0.1785, 0.7485), ("2", "0"), "スペイン軸 + 条件戦ドロー", "semi"),
    PlanMatch(4, "コロンビア", "ポルトガル", "06/28 08:30", (0.2630, 0.3057, 0.4313), ("2", "0", "1"), "30%台を全分散", "spread"),
    PlanMatch(5, "アルジェリア", "オーストリア", "06/28 11:00", (0.1922, 0.2902, 0.5176), ("2", "0"), "オーストリア軸 + ドロー", "flex"),
    PlanMatch(6, "チュニジア", "オランダ", "06/26 08:00", (0.0268, 0.0423, 0.9309), ("2", "0"), "90%超人気 + 薄いドロー", "semi"),
    PlanMatch(7, "パラグアイ", "オーストラリア", "06/26 11:00", (0.3669, 0.3101, 0.3230), ("1", "0", "2"), "ほぼ三分で全分散", "spread"),
    PlanMatch(8, "ノルウェー", "フランス", "06/27 04:00", (0.0898, 0.1647, 0.7455), ("2", "0"), "フランス軸 + 条件戦ドロー", "semi"),
    PlanMatch(9, "パナマ", "イングランド", "06/28 06:00", (0.0234, 0.0403, 0.9363), ("2",), "補正ありでも勝ち固定", "lock"),
    PlanMatch(10, "コンゴ民主共和国", "ウズベキスタン", "06/28 08:30", (0.3955, 0.3402, 0.2643), ("1", "0", "2"), "30%台に散る全分散", "spread"),
    PlanMatch(11, "ヨルダン", "アルゼンチン", "06/28 11:00", (0.0215, 0.0340, 0.9445), ("2",), "補正ありでも勝ち固定", "lock"),
    PlanMatch(12, "ニュージーランド", "ベルギー", "06/27 12:00", (0.0402, 0.0911, 0.8687), ("2", "0"), "強豪人気 + 薄いドロー", "semi"),
    PlanMatch(13, "クロアチア", "ガーナ", "06/28 06:00", (0.7620, 0.1579, 0.0801), ("1", "0"), "クロアチア軸 + 条件戦ドロー", "semi"),
]
...
POLYMARKET_1637_ROWS = [
    ...
    {
        "no": 11,
        "market": (0.0739, 0.1133, 0.8128),
        "delta": (0.0524, 0.0793, -0.1317),
        "volume": 45_781,
...
MARKET_ADJUSTED_MULTI_PLANS_1637 = [
    {
        "label": "市場補強27口",
        "choices": ("2/0/1", "1", "2", "2", "2/0/1", "2", "1/0/2", "2", "2", "1", "2", "2", "1"),
        "note": "M01/M05/M07だけ市場差分を強く反映した最小形。",
    },
    {
        "label": "市場補強54口",
        "choices": ("2/0/1", "1", "2", "2", "2/0/1", "2", "1/0/2", "2", "2", "1", "2", "2", "1/0"),
        "note": "M13のドローを追加。Croatia人気の重さをケア。",
    },
    {
        "label": "市場補強108口",
        "choices": ("2/0/1", "1/0", "2", "2", "2/0/1", "2", "1/0/2", "2", "2", "1", "2", "2", "1/0"),
...
def final_market_decision_rule_table() -> Table:
    rows = [
        ...
        ["200口以内", "優先順は M01/M05/M07/M13 → 日本戦M02 → 低流動性のM08/M09/M11/M12。"],
    ]
...
def final_logic_table_1637() -> Table:
    rows = [
        ...
        ["暫定確定", "締切直前も差分が同じなら市場補強108口=10,800円を優先。広めは162口=16,200円。"],
...
def build_pdf() -> Path:
    ...
    story = [
        ...
        p("Haziコメントは無しとして、人間メモ重みは0にします。ここではPolymarketの公開Sports APIから取得できたW杯1X2価格をp_marketとして、toto公式投票率p_publicとの差を見ます。価格はbid/ask中点を3出目合計100%へ正規化したものです。取得は2026-06-22 20:57 JST相当です。"),
