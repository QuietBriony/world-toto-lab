...
6. `公式対象回ライブラリ / CSV` -> `toto-official-round-import`
...
- GitHub Actions の annotation として `actions/configure-pages@v5` の Node 20 deprecation warning は残ります
- これは GitHub 公式 action 側の最新 major がまだ `v5` のため、repo 側では解消不可です
