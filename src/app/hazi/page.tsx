...
function modelSourceLabel(value: HaziLiteRound["matches"][number]["modelSource"]) {
  if (value === "team_strength_with_official_crowd") {
    return "強度 + 公式crowd";
  }

  if (value === "official_vote") {
    return "公式人気";
  }
...
export default function HaziPage() {
  ...
  const usesSharedD1 =
    ...
    Boolean(process.env.NEXT_PUBLIC_D1_API_BASE);
              ...
              <article key={match.matchId} className="px-4 py-4">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">
                    {modelSourceLabel(match.modelSource)}
                  </span>
