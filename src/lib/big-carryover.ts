...
export const bigEventTypeDescription: Record<BigEventType, string> = {
  carryover_event: "大きなキャリーが現在売上に対してどれくらい重いかを見る管理モードです。",
  high_return_watch: "売上とキャリーから粗い上振れ指標を見張る管理モードです。真EVは別途ルール確認が必要です。",
};
...
export const bigCarryoverPresets: BigCarryoverPreset[] = [
  ...
  },
  {
    ...
    description: "かなり厚いキャリーが乗った話題回を、キャリー圧として再現します。",
    eventLabel: "BIG 大型キャリー仮定",
    eventType: "carryover_event",
    id: "hot-event",
    returnRatePercent: 50,
    salesYen: 8_000_000_000,
...
export function buildBigShockAlert(input: {
  ...
  if (input.signal === "strong") {
    if (input.summary.approxEvMultiple !== null && input.summary.approxEvMultiple >= 1) {
      return {
        badgeTone: "positive",
        hint: "キャリー圧に加えて、中止・返還・成立条件の変化を疑うメモがあります。公式告知の確認を優先します。",
        label: "成立条件要確認",
      };
    }
