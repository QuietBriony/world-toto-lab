...
export const demoRoundTitle = "Demo Tour Round";
export const demoRoundNotes =
  "チュートリアル用の仮Roundです。AI Base、Human Overlay、Consensus、Edge、Tickets、Review まで一通り入っています。金銭、配当、代理購入、精算は扱いません。";
...
const demoMatchSeeds: DemoMatchSeed[] = [
  ...
  },
  {
    ...
    adminNote: "Review で drawAlert を見返しやすい。",
    actualResult: "DRAW",
  },
  {
    matchNo: 10,
  ...
  return [
    {
      ...
      user_id: reporterA,
      note: "AI が 0/2 を出している試合では、人力で 0 を足したメンバーの視点が Review でも効いていました。",
    },
    {
      round_id: roundId,
      match_id: matchByNo.get(5)?.id ?? null,
      user_id: reporterB,
      note: "人気先行の 1 に対して 2 を差し込んだ判断が刺さった例。AI Base を見たうえで別筋を作る意義が分かります。",
    },
    {
      round_id: roundId,
      match_id: matchByNo.get(11)?.id ?? null,
      user_id: reporterC,
      note: "info_wait は広く持つのが正解でした。デモでは exceptionFlag と Review のつながりを確認できます。",
    },
    {
      round_id: roundId,
      match_id: null,
      user_id: reporterA,
      note: "この Demo Round は、AI Base -> Human Overlay -> Consensus -> Ticket Generator -> Review の順で触ると全体像が掴みやすいです。",
    },
